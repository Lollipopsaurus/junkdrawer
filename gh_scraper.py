...
def scrape_url(url, file_name):
    ...
    if etf_main_post:
        ...
        if this_post_md5+'\n' not in stored_posts and len(stored_posts) > 0:
            alert_response.append('<@525716579179298816> ETF EDITED HIS POST ON GEEKHACK ' + url + ' ' + etf_main_post.text)
    write_temp(to_store_posts, file_name)
    return alert_response

# Main that does stuff
def main(user):
    alert_response = scrape_url('https://geekhack.org/index.php?topic=79513.msg2048390#msg2048390', 'user_data/gh_etf.txt')
    return alert_response
